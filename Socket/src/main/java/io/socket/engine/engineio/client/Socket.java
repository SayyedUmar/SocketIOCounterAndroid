...
import io.socket.engine.engineio.client.transports.PollingXHR;
import io.socket.engine.engineio.client.transports.WebSocket;
import io.socket.engine.engineio.parser.Packet;
import io.socket.engine.engineio.parser.Parser;
import io.socket.engine.parseqs.ParseQS;
...
public class Socket extends Emitter {
    ...
    public Socket(Options opts) {
        ...
        if (callFactory == null) {
            callFactory = (Call.Factory) getDefaultOkHttpClient();
        }
        if (webSocketFactory == null) {
            webSocketFactory = (okhttp3.WebSocket.Factory) getDefaultOkHttpClient();
        }
